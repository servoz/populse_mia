...
class Project:
    ...
    def __init__(self, project_root_folder, new_project):
        ...
        if self.folder not in opened_projects:
            ...
            raise IOError(
                "The project at " + str(
                    self.folder) + " is already opened in another instance of the software.")
        ...
        if new_project:
            ...
            with open(
                    ...
                    'w', encoding='utf8') as propertyfile:
                yaml.dump(properties, propertyfile, default_flow_style=False,
                          allow_unicode=True)
    ...
    def add_clinical_tags(self):
        ...
        :return: clinical tags that were added
        """
        return_tags = []
        for clinical_tag in CLINICAL_TAGS:
            if clinical_tag not in self.session.get_fields_names(
                ...
                for scan in self.session.get_documents(COLLECTION_CURRENT):
                    self.session.add_value(COLLECTION_CURRENT,
                                           getattr(scan, TAG_FILENAME),
                                           clinical_tag,
                                           None)
                    self.session.add_value(COLLECTION_INITIAL,
                                           getattr(scan, TAG_FILENAME),
                                           clinical_tag,
                                           None)
    ...
    def getFilterName(self):
        ...
        text, ok_pressed = QInputDialog.getText(None, "Save a filter",
                                                "Filter name: ",
                                                QLineEdit.Normal, "")
    ...
    def init_filters(self):
        ...
        for filename in glob.glob(os.path.join(filters_folder, '*')):
            ...
            with open(filename,
                      'r') as f:  # make sure this gets closed automatically as soon as we are done reading
                data = json.load(f)
    ...
    def redo(self, table):
        ...
        if len(self.redos) > 0:
            ...
            self.undos.append(
                to_redo)  # We pop the redo action in the undo stack
            ...
            if action == "add_tag":
                ...
                # Adding the tag
                self.session.add_field(COLLECTION_CURRENT, tag_to_add,
                                       tag_type, tag_description, True,
                                       TAG_ORIGIN_USER, tag_unit,
                                       tag_default_value)
                self.session.add_field(COLLECTION_INITIAL, tag_to_add,
                                       tag_type, tag_description, True,
                                       TAG_ORIGIN_USER, tag_unit,
                                       tag_default_value)
                # Adding all the values associated
                for value in values:
                    self.session.add_value(COLLECTION_CURRENT, value[0],
                                           value[1], value[2])
                    self.session.add_value(COLLECTION_INITIAL, value[0],
                                           value[1], value[3])
            ...
            if action == "remove_tags":
                ...
                tags_removed = to_redo[
                    1]  # The second element is a list of the removed tags (Tag class)
            ...
            if action == "add_scans":
                ...
                scans_added = to_redo[
                    1]  # The second element is a list of the scans to add
                ...
                values_added = to_redo[
                    2]  # The third element is a list of the values to add
                for i in range(0, len(values_added)):
                    value_to_add = values_added[i]
                    self.session.add_value(COLLECTION_CURRENT, value_to_add[0],
                                           value_to_add[1], value_to_add[2])
                    self.session.add_value(COLLECTION_INITIAL, value_to_add[0],
            ...
            if action == "remove_scans":
                ...
                scans_removed = to_redo[
                    1]  # The second element is the list of removed scans (Path class)
                for i in range(0, len(scans_removed)):
                    ...
                    self.session.remove_document(COLLECTION_CURRENT,
                                                 scan_to_remove)
                    self.session.remove_document(COLLECTION_INITIAL,
                                                 scan_to_remove)
            ...
            if action == "modified_values":  # Not working
                ...
                modified_values = to_redo[
                    1]  # The second element is a list of modified values (reset or value changed)
                table.itemChanged.disconnect()
                for i in range(0, len(modified_values)):
                    ...
                    item = table.item(table.get_scan_row(scan),
                                      table.get_tag_column(tag))
                    if old_value is None:
                        ...
                        font.setBold(False)
                        item.setFont(font)
                    self.session.set_value(COLLECTION_CURRENT, scan, tag,
                                           new_value)
                    if new_value is None:
                        ...
                        set_item_data(item, new_value, self.session.get_field(
            ...
            if action == "modified_visibilities":
                ...
                showed_tags = to_redo[
                    2]  # List of the tags shown before the modification (Tag objects)
    ...
    def reput_values(self, values):
        ...
        for i in range(0, len(values)):
            ...
            self.session.add_value(COLLECTION_CURRENT, valueToReput[0],
                                   valueToReput[1], valueToReput[2])
            self.session.add_value(COLLECTION_INITIAL, valueToReput[0],
                                   valueToReput[1], valueToReput[3])
        ...
        if filter_name != None:
            ...
            if os.path.exists(file_path):
                ...
                with open(file_path, 'w') as outfile:
                    new_filter = Filter(filter_name, self.currentFilter.nots,
    ...
    def saveConfig(self):
        ...
        with open(os.path.join(self.folder, 'properties', 'properties.yml'),
            ...
            yaml.dump(self.properties, configfile, default_flow_style=False,
                      allow_unicode=True)
    ...
    def undo(self, table):
        ...
        if len(self.undos) > 0:
            ...
            self.redos.append(
                to_undo)  # We pop the undo action in the redo stack
            ...
            if action == "remove_tags":
                ...
                tags_removed = to_undo[
                    1]  # The second element is a list of the removed tags ([Tag row, origin,
                # unit, default_value])
                for i in range(0, len(tags_removed)):
                    # We reput each tag in the tag list, keeping all the tags params
                    ...
                    self.session.add_field(COLLECTION_INITIAL,
                                           tag_to_reput.field_name,
                                           tag_to_reput.type,
                                           tag_to_reput.description,
                                           tag_to_reput.visibility,
                                           ...
                                           tag_to_reput.unit,
                                           tag_to_reput.default_value)
                values_removed = to_undo[
                    2]  # The third element is a list of tags values (Value class)
            ...
            if action == "add_scans":
                ...
                scans_added = to_undo[
                    1]  # The second element is a list of added scans to remove
                for i in range(0, len(scans_added)):
                    # We remove each scan added
            ...
            if action == "remove_scans":
                ...
                scans_removed = to_undo[
                    1]  # The second element is the list of removed scans (Scan class)
                for i in range(0, len(scans_removed)):
                    # We reput each scan, keeping the same values
                    scan_to_reput = scans_removed[i]
                    self.session.add_document(COLLECTION_CURRENT,
                                              getattr(scan_to_reput,
                                                      TAG_FILENAME))
                    self.session.add_document(COLLECTION_INITIAL,
                                              getattr(scan_to_reput,
                                                      TAG_FILENAME))
                    table.scans_to_visualize.append(
                        getattr(scan_to_reput, TAG_FILENAME))
                values_removed = to_undo[
                    2]  # The third element is the list of removed values
                self.reput_values(values_removed)
                table.add_rows(
                    self.session.get_documents_names(COLLECTION_CURRENT))
            if action == "modified_values":
                ...
                modified_values = to_undo[
                    1]  # The second element is a list of modified values (reset,
                # or value changed)
                table.itemChanged.disconnect()
                for i in range(0, len(modified_values)):
                    # Each modified value is a list of 3 elements: scan, tag, and old_value
                    ...
                    if old_value is None:
                        ...
                        self.session.remove_value(COLLECTION_CURRENT, scan,
                                                  tag)
                        self.session.remove_value(COLLECTION_INITIAL, scan,
                                                  tag)
                        set_item_data(item, not_defined_value,
                                      FIELD_TYPE_STRING)
                        font = item.font()
                        ...
                        self.session.set_value(COLLECTION_CURRENT, scan, tag,
                                               old_value)
                        set_item_data(item, old_value, self.session.get_field(
                            COLLECTION_CURRENT, tag).type)
